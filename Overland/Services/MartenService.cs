...
public class MartenService : IMartenService {
    ...
    public async Task<Promocode?> GetPromocode(Guid guid) {
        try {
            await using var session = _store!.LightweightSession();
            var result = session.Load<Promocode>(guid);

            return result;
        }
        catch (Exception ex) {
            _logger.Error("Unable to retrieve promocode for {@ex} {@guid}", ex, guid);
            return null;
        }
    }
